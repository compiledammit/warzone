			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="addedOn" title="${message(code: 'feed.addedOn.label', default: 'Added On')}" />
					
						<g:sortableColumn property="lastUpdatedOn" title="${message(code: 'feed.lastUpdatedOn.label', default: 'Last Updated On')}" />
				...
				<g:each in="${feedInstanceList}" status="i" var="feedInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:formatDate date="${feedInstance.addedOn}" /></td>
					
						<td><g:formatDate date="${feedInstance.lastUpdatedOn}" /></td>
